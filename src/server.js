...
const chromeIcons = {
  ...
  // LOCAL ADDITION: compact entrypoint for the collapsed review dock.
  controls: chromeIcon(
    '<line x1="4" y1="7" x2="20" y2="7"/><circle cx="9" cy="7" r="2"/><line x1="4" y1="17" x2="20" y2="17"/><circle cx="15" cy="17" r="2"/>',
    15,
  ),
  // LOCAL ADDITION: side-panel collapse toggle (Cmd/Ctrl+\).
  panel: chromeIcon('<rect x="3" y="4" width="18" height="16" rx="2"/><line x1="15" y1="4" x2="15" y2="20"/>', 14),
  refresh: chromeIcon(
...
export function createChromeHtml(
  ...
  const endHotkeyUpper = END_SESSION_HOTKEY_KEY.toUpperCase();
  const endSessionHint = `End session (⇧⌘${endHotkeyUpper} / Ctrl+Shift+${endHotkeyUpper})`;
  // LOCAL ADDITION: side-panel collapse hint. Escaped at the use site like modeToggleHint.
  const panelToggleHint = escapeHtml("⌘\\ / Ctrl+\\");
  return `<!doctype html>
<html>
<head>
<meta charset="utf-8">
...
<div class="bar" id="reviewDock" role="toolbar" aria-label="Review controls">
  <button class="bar-toggle" id="barToggle" type="button" aria-controls="barControls" aria-expanded="false" aria-label="Show review controls" title="Show review controls">${chromeIcons.controls}<span class="bar-warning-count" id="barWarningCount" hidden>0</span></button>
