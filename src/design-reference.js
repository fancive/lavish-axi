...
export function createDesignOutput() {
  return {
    ...
    design: {
      // LOCAL PATCH: the Tailwind/DaisyUI CDN fallback is removed entirely. Shipping the snippet
      // and its urls while telling the agent "do not use a CDN runtime" is a contradictory
      // instruction - in practice the agent follows the concrete snippet, not the prohibition.
    ...
    // LOCAL ADDITION (not upstream): the CDN snippet is replaced by a local compile step.
    ...
    // LOCAL PATCH: upstream defaulted to the `luxury` theme, which is dark. This install is
    // light-first (see chrome.css and build-css.mjs), and the local build only compiles the
    // themes it is told to, so the guidance and the build have to agree.
    theme_usage: [
      ...
      "Never `@apply` DaisyUI classes (such as `text-base-content/40`, `bg-base-200`, or `btn`) - Tailwind does not know them, and one unknown utility aborts the whole compile. Put DaisyUI classes directly on elements, or write plain CSS with theme variables such as `var(--color-base-200)`. With the local build this fails loudly at build time instead of silently at view time, but it still fails.",
    ],
