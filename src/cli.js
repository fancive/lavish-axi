...
export function createHomeOutput({ bin, sessions, includeSessions = true, agent = "generic" }) {
  return {
    ...
    visual_guidance: [
      ...
      // LOCAL PATCH: removed "(run the app read-only if needed)" - the agent must not start or run
      // the user's application just to take screenshots.
      "When the artifact would describe existing or current UI or state, prefer showing it: embed screenshots only if they already exist or the user provides them - never start, run, or drive the app to produce them. Otherwise describe the current look in prose; reserve prose for what cannot be shown such as rationale, trade-offs, and open questions",
    ],
    ...
    help: [
      ...
      // LOCAL PATCH: upstream default was the project working directory under `.lavish/`, which
      // pollutes the repo. Artifacts belong in /tmp.
      "Unless the user specifies another location, create HTML artifacts under `/tmp/`. NEVER write them into the project working directory or any git repository - not `.lavish/`, not anywhere else in the repo",
...
function createCommandHelp({ agent = "generic" } = {}) {
  return {
    ...
    // text must describe what the command actually emits - the earlier version kept advertising
    // a snippet that had already been removed, which just sends the agent hunting for it.
    design: `Usage: lavish-axi design\n\nShow Mermaid diagram tooling, a content-to-playbook router, an optional layout safety CSS snippet, plus the local Tailwind+DaisyUI build command. ${PLAYBOOK_ROUTER_HELP} Lavish artifacts stay portable HTML. LOCAL PATCH: this install has no CDN fallback. Tailwind + DaisyUI are still the vocabulary, but they are compiled locally per artifact into a sibling .css file (only the classes actually used, ~20KB) instead of fetched at view time. Paste the layout safety CSS only when useful for dense nested grid/flex layouts, badges, wide fonts, or local media. ${DESIGN_PRIORITY_RULE}\n`,
