...
function createCommandHelp({ agent = "generic" } = {}) {
  return {
    ...
    // LOCAL PATCH: the Tailwind/DaisyUI CDN snippet and component reference are removed from
    // this install, so this help text no longer advertises them - a help string promising a
    // snippet the command does not emit just sends the agent hunting for it elsewhere.
    design: `Usage: lavish-axi design\n\nShow Mermaid diagram tooling, a content-to-playbook router, an optional layout safety CSS snippet, plus the styling rules for hand-written self-contained CSS. ${PLAYBOOK_ROUTER_HELP} Lavish artifacts stay portable HTML. LOCAL PATCH: this install has no Tailwind/DaisyUI CDN fallback - the fallback tier is hand-written inline CSS. Paste the layout safety CSS only when useful for dense nested grid/flex layouts, badges, wide fonts, or local media. ${DESIGN_PRIORITY_RULE}\n`,
