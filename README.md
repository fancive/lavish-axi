...
- **Layout issues inbox** - Detection is passive. After fonts and finite animations settle, the injected SDK confirms severe failures from direct rendered evidence such as materially escaped meaningful content or required controls, clipped text fragments, viewport reachability, or near-total semantic occlusion.
  ...
  Proven failures are filed in the **Layout issues** inbox inside the bottom-right review dock. The dock keeps the Annotate switch visible, collapses the low-frequency controls by default, mirrors the unresolved count on its entrypoint, and expands on demand without taking height from or covering the artifact header.
...
- **Feedback controls** - Native controls (radios, checkboxes, inputs, selects, buttons, labels, disclosure summaries, contenteditable) are interactive automatically, so they do not need `data-lavish-action`.
  ...
  The browser chrome lives in a bottom-right review dock. Annotate and its current mode stay visible so the Cmd/Ctrl+I state is always apparent; expanding the dock reveals the conversation-panel toggle, the editing overflow menu (copy path, reload artifact, copy DOM snapshot, export standalone HTML, publish link), and a high-contrast **End session** action. The composer exposes **Send & End** beside **Send to Agent** to submit queued prompts and user-ended attribution together.
